...
`cov_exp_quad` takes the following arguments:
...
`cov_rat_quad` takes the following arguments:
...
`cov_periodic` takes the following arguments:
...
`cov_linear` takes the following arguments:
...
`cov_noise` takes the following arguments:

* `xa` --- vector of values
* `xb` --- vector of values
* `sigma` --- scalar denoting the variance. Defaults to `0.5`
...
```{r, message = FALSE, warning = FALSE}
mod <- GP(x1, seq(from = 1, to = 100, by = 5), y, CovSum, 1e-9,
           sigma_1 = 5, sigma_2 = 1, sigma_3 = 2, 
           l_1 = 75, l_2 = 1, p = 25)
...
Here is another for a model which predicts $50$ time points but with $1000$ draws:

```{r, message = FALSE, warning = FALSE}
mod2 <- GP(x1, seq(from = 1, to = 100, by = 2), y, CovSum, 1e-9,
           sigma_1 = 5, sigma_2 = 1, sigma_3 = 2, 
           l_1 = 75, l_2 = 1, p = 25)
