...
Before we define the GP itself, for ease and clarity, we are going to construct a composite kernel of a trend, seasonality, and noise. `tsgp` has a function `composite` for doing exactly this. `composite` only takes two arguments:

1. `kernels` --- a list of all the individual covariance functions
2. `combine` --- character specifying whether to add (`combine = "add"`) or multiply (`combine = "mul"`) the kernels together

`composite` also returns on object of class `GPCov` so that the composite matrix can be readily used with the other package generic functions
...
* `covfun` --- covariance function to use
