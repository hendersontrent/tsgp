...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{Gaussian processes for time-series analysis}
...
*NOTE: This vignette is a condensed version of a deeper treatment of how GPs can model time-series data which is available to be read [here](https://hendersontrent.github.io/posts/2024/05/gaussian-process-time-series/).*
...
Before we define the GP itself, for ease and clarity, we are going to define a simple function which automatically computes and sums the covariance functions we want.
...
Okay, we can now use the core `GP` function in `tsgp` to calculate the posterior mean vector and covariance matrix. `GP` returns an object of class `x` which is a list of the input data, the posterior mean vector, and the posterior covariance matrix. `GP` takes the following arguments:
...
* `epsilon` --- small quantity to add to the $(x, x)$ covariance matrix for numerical stability. Defaults to $1e^{-9}$
* `...` --- hyperparameters to be passed to `cov_function`

Here is an example for a model which predicts values for every time point:

```{r, message = FALSE, warning = FALSE}
mod <- GP(x1, 1:length(y), y, CovSum, 1e-9,
           sigma_1 = 5, sigma_2 = 1, sigma_3 - 0.8, 
           l_1 = 75, l_2 = 1, p = 25)
```

We can easily visualise draws from the posterior distribution by calling `plot()` on a `TSGP` object:

```{r, message = FALSE, warning = FALSE}
plot(mod, 0.95, 100)
```

The `plot()` generic for `TSGP` takes only three arguments:
...
What happens if we increase the noise (i.e., uncertainty)?

```{r, message = FALSE, warning = FALSE}
mod2 <- GP(x1, 1:length(y), y, CovSum, 1e-9,
           sigma_1 = 5, sigma_2 = 1, sigma_3 = 1.2, 
           l_1 = 75, l_2 = 1, p = 25)

plot(mod2, 0.95, 100)
```

Intuitively, we see that the prediction intervals widen (i.e., become more uncertain) with the greater inclusion of noise.

Finally, `tsgp` can also produce plausible short-term-horizon forecasts on out-of-sample time points:

```{r, message = FALSE, warning = FALSE}
mod3 <- GP(x1, length(y):150, y, CovSum, 1e-9,
           sigma_1 = 5, sigma_2 = 1, sigma_3 = 1.2, 
           l_1 = 75, l_2 = 1, p = 25)
